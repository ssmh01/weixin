 ...
 */
class LastRangePlayWayAdapter extends IPlayWayAdapter{
	/*
	...
	public function resultRudge(PlayData $playData){
		...
		if($parameter->isEmptyValue() || $chooseValue == '' || !$this->isRangeValue($chooseValue)){
			return false;
		}
		...
		if($chooseOods == 0){
			...
			$isCorrect = $this->inRangeValue($lastNumber, $chooseValue);
			if($isCorrect){
